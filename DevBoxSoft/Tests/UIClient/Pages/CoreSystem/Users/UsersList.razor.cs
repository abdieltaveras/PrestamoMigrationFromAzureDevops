...
namespace UIClient.Pages.CoreSystem.Users
{
    public partial class UsersList : BasePage
    {
        ...
        {
        new DataGridViewToolbarButton(){ Color= Color.Primary, Icon=Icons.Filled.AddCircle, Text="Nuevo", OnClick=btnAddClick, IsEnabled=btnAddEnabled},
        new DataGridViewToolbarButton(){ Color= Color.Secondary, Icon=Icons.Filled.Edit, Text="Modificar", OnClick=btnEdtClick, IsEnabled=btnEdtEnabled},
        new DataGridViewToolbarButton(){ Color= Color.Tertiary, Icon=Icons.Filled.VpnKey, Text="Debe Cambiar Contraseña", OnClick=btnChgPwdClick, IsEnabled=btnEdtEnabled},
        };
        ...
        private bool filterFunc(object obj)
        {
            ...
            if (group.GroupName.Contains(searchString, StringComparison.OrdinalIgnoreCase))
                return true;
            if (group.FullName.Contains(searchString, StringComparison.OrdinalIgnoreCase))
                return true;
            if (!group.NationalID.IsNullOrEmpty() && group.NationalID.Contains(searchString, StringComparison.OrdinalIgnoreCase))
                return true;
            if (group.UserName.Contains(searchString, StringComparison.OrdinalIgnoreCase))
                return true;
            return false;
        }
    }
