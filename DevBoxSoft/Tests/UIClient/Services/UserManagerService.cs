...
namespace UIClient.Services
{
    public class UserManagerService : HttpServiceBase
    {
        ...
        {            
            await PostAsync<object, object>("api/user/changepwd", new { id, password });
        }
        public async Task ResetUserPassword(string NationalID)
        {
            await PostAsync<object, object>("api/user/resetpassword", new { NationalID });
        }
        public async Task ForceResetUserPassword(Guid id)
        {
            await PostAsync<object, object>("api/user/forceresetpassword", new { id });
        }
