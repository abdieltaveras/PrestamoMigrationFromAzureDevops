...
namespace DevBox.Core.Access
{
    public class ActionList : ICollection<Action>, IEnumerable<Action>
    {
        ...
        public class ActionComparter : IComparer<Action>
        {
            private ActionSort sortType = ActionSort.byName;
            public ActionComparter(ActionSort sortType)
            {
        ...
        ActionPermissionLevel[] permissionFilter = new ActionPermissionLevel[0]; //ActionList.actionFilters[ActionListFilters.All];
        List<Action> list
        {
            get { return _list; }
            set { _list = value; }
        ...
        {
            this.list.Sort(new ActionComparter(sortType));
        }
        ...
        public Action this[string displayName]
        {
            get
            {
                Action result = list.Where(a => a.DisplayName.Equals(displayName, StringComparison.InvariantCultureIgnoreCase)).SingleOrDefault(); ;
                return result;
            }
            set
            {
