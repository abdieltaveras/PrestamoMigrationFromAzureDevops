...
namespace DevBox.Core.Access
{
    public class SubActionList : IList<SubAction>
    {
        ...
        public SubAction this[string SubActionName]
        {
            get
            {
                var result = (SubAction)(from a in this.SubActions where (string.Compare(a.Value, SubActionName, true) == 0) select a).SingleOrDefault();
                if (result == null)
                {
